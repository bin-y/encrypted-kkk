...
var sessionDirectory = app.getPath('temp') + '/ek-sessions/';
...
while (!sessionDirectoryCreated) {
    var randomString = crypto.randomBytes(8).toString('hex');
    try {
        fs.accessSync(sessionDirectory + randomString);
    } catch (error) {
        sessionDirectory += randomString;
